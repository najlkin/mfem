...
{
   if (!static_cond && mat) { mat->Finalize(skip_zeros); }
...
void BilinearForm::ComputeFaceElementMatrix(int i, DenseMatrix &elmat)
{
   ...
   if (fbfi.Size())
   {
      ...
      fbfi[0] -> AssembleFaceMatrix (*fe1, *fe2, *tr, elmat);
      for (int k = 1; k < fbfi.Size(); k++)
      {
         fbfi[k] -> AssembleFaceMatrix (*fe1, *fe2, *tr, elemmat);
         elmat += elemmat;
      }
   }
   else
...
void BilinearForm::ComputeBdrFaceElementMatrix(int i, DenseMatrix &elmat)
{
   ...
   if (bfbfi.Size())
   {
      ...
      bfbfi[0] -> AssembleFaceMatrix (*fe1, *fe2, *tr, elmat);
      for (int k = 1; k < bfbfi.Size(); k++)
      {
...
void MixedBilinearForm::Assemble (int skip_zeros)
{
   ...
   if (dbfi.Size())
   {
      for (int i = 0; i < test_fes -> GetNE(); i++)
      {
         ...
         if (element_matrices)
         {
            mat -> AddSubMatrix(te_vdofs, tr_vdofs, (*element_matrices)(i), skip_zeros);
         }
         else
         {
   ...
   if (fbfi.Size())
   {
      ...
      for (int i = 0; i < nfaces; i++)
      {
         ...
         if (ftr != NULL)
         {
            ...
            for (int k = 0; k < fbfi.Size(); k++)
            {
               fbfi[k]->AssembleFaceMatrix(*trial_fe1, *test_fe1, *trial_fe2, *test_fe2,
                                           *ftr, elemmat);
               mat->AddSubMatrix(te_vdofs, tr_vdofs, elemmat, skip_zeros);
            }
         }
      ...
      for (int i = 0; i < trial_fes -> GetNBE(); i++)
      {
         ...
         if (ftr != NULL)
         {
            ...
            for (int k = 0; k < bfbfi.Size(); k++)
            {
               ...
               bfbfi[k]->AssembleFaceMatrix(*trial_fe1, *test_fe1, *trial_fe2, *test_fe2,
                                            *ftr, elemmat);
               mat->AddSubMatrix(te_vdofs, tr_vdofs, elemmat, skip_zeros);
            }
         }
   ...
   if (btfbfi.Size())
   {
      ...
      for (int i = 0; i < trial_fes -> GetNBE(); i++)
      {
         ...
         if (ftr)
         {
            trial_fes->GetFaceVDofs(ftr->Face->ElementNo, tr_vdofs);
            test_fes->GetElementVDofs(ftr->Elem1No, te_vdofs);
...
{
   if (element_matrices || dbfi.Size() == 0 || test_fes->GetNE() == 0)
   {
      return;
   }

   int num_elements = test_fes->GetNE();
   int nvdofs_tr = trial_fes->GetFE(0)->GetDof() * trial_fes->GetVDim();
   int nvdofs_te = test_fes->GetFE(0)->GetDof() * test_fes->GetVDim();

   element_matrices = new DenseTensor(nvdofs_te, nvdofs_tr,
                                      num_elements);
   ...
   for (int i = 0; i < num_elements; i++)
   {
...
#ifdef MFEM_DEBUG
      if (nvdofs_tr != trial_fe.GetDof() * trial_fes->GetVDim()
          || nvdofs_te != test_fe.GetDof() * test_fes->GetVDim())
         mfem_error("BilinearForm::ComputeElementMatrices:"
                    " all elements must have same number of dofs");
#endif
      ...
      {
         // note: some integrators may not be thread-safe
         dbfi[k]->AssembleElementMatrix2(trial_fe, test_fe, eltrans, tmp);
         elmat += tmp;
      }
      elmat.ClearExternalData();
   }
...
void MixedBilinearForm::ComputeFaceElementMatrix(int i, DenseMatrix &elmat)
{
   ...
   if (fbfi.Size())
   {
      ...
      fbfi[0]->AssembleFaceMatrix(*trial_fe1, *test_fe1, *trial_fe2, *test_fe2,
                                  *ftr, elmat);
      for (int k = 1; k < fbfi.Size(); k++)
      {
         fbfi[k]->AssembleFaceMatrix(*trial_fe1, *test_fe1, *trial_fe2, *test_fe2,
...
void MixedBilinearForm::ComputeBdrFaceElementMatrix(int i, DenseMatrix &elmat)
{
   ...
   if (bfbfi.Size())
   {
      ...
      bfbfi[0]->AssembleFaceMatrix(*trial_fe1, *test_fe1, *trial_fe2, *test_fe2,
                                   *ftr, elmat);
      for (int k = 1; k < bfbfi.Size(); k++)
      {
         bfbfi[k]->AssembleFaceMatrix(*trial_fe1, *test_fe1, *trial_fe2, *test_fe2,
...
void MixedBilinearForm::ComputeTraceFaceElementMatrix(int i, DenseMatrix &elmat)
{
   ...
   if (tfbfi.Size())
   {
      ...
      tfbfi[0]->AssembleFaceMatrix(*trial_face_fe, *test_fe1, *test_fe2,
                                   *ftr, elmat);
      for (int k = 1; k < tfbfi.Size(); k++)
      {
         tfbfi[k]->AssembleFaceMatrix(*trial_face_fe, *test_fe1, *test_fe2,
                                      *ftr, elemmat);
         elmat += elemmat;
                                                         ...
                                                         DenseMatrix &elmat)
{
   ...
   if (btfbfi.Size())
   {
      ...
      // want to actually make a fake element.
      test_fe2 = test_fe1;
      btfbfi[0]->AssembleFaceMatrix(*trial_face_fe, *test_fe1, *test_fe2,
                                    *ftr, elmat);
      for (int k = 1; k < btfbfi.Size(); k++)
      {
         btfbfi[k]->AssembleFaceMatrix(*trial_face_fe, *test_fe1, *test_fe2,
                                       *ftr, elemmat);
         elmat += elemmat;
