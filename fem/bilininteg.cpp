...
void DGTraceIntegrator::AssembleFaceMatrix(const FiniteElement &trial_fe1,
                                           ...
                                           DenseMatrix &elmat)
{
   int dim, tr_ndof1, te_ndof1, tr_ndof2, te_ndof2;
   ...
   DenseMatrix &elmat)
{
   ...
   tangent.SetSize(dim);
   shape1.SetSize(ndof1,dim);
   shape1_t.SetSize(ndof1);

   if (Trans.Elem2No >= 0)
   {
      ndof2 = test_fe2.GetDof();
      shape2.SetSize(ndof2,dim);
      shape2_t.SetSize(ndof2);
   }
   else
   {
      ndof2 = 0;
   ...
   for (int p = 0; p < ir->GetNPoints(); p++)
   {
      ...
      trial_face_fe.CalcShape(ip, face_shape);
      Trans.Loc1.Transf.SetIntPoint(&ip);
      Trans.Loc1.Transf.Jacobian().GetColumn(0, tangent);
      // Side 1 finite element shape function
      Trans.Loc1.Transform(ip, eip1);
      test_fe1.CalcVShape(eip1, shape1);
      shape1.Mult(tangent, shape1_t);
      if (ndof2)
      {
         // Side 2 finite element shape function
         Trans.Loc2.Transform(ip, eip2);
         test_fe2.CalcVShape(eip2, shape2);
         Trans.Loc2.Transf.SetIntPoint(&ip);
         Trans.Loc2.Transf.Jacobian().GetColumn(0, tangent);
         shape2.Mult(tangent, shape2_t);
      }
      face_shape *= ip.weight;
      for (i = 0; i < ndof1; i++)
         for (j = 0; j < face_ndof; j++)
         {
            elmat(i, j) -= shape1_t(i) * face_shape(j);
         }
      if (ndof2)
      {
         // Subtract contribution from side 2
         for (i = 0; i < ndof2; i++)
            for (j = 0; j < face_ndof; j++)
            {
               elmat(ndof1+i, j) += shape2_t(i) * face_shape(j);
            }
      }
   }
}
